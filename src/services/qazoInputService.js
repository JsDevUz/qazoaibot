...
class QazoInputService {
  constructor(bot, qazoService, userService) {
    this.bot = bot;
  ...
  async addQazoToDatabase(telegramId, qazoData) {
    try {
      ...
      await this.qazoService.getOrCreateQazoCount(telegramId);
      ...
      for (const [prayer, count] of Object.entries(qazoData)) {
        updateField[`${prayer}_count`] = count;
        totalAdded += count;
      }
      updateField["total_count"] = totalAdded;

      const QazoCount = require("../models/QazoCount");
      await QazoCount.findOneAndUpdate(
        { user_id: telegramId },
        { $inc: updateField },
      );

      console.log(`Updated qazo counts for user ${telegramId}:`, qazoData);
      return await this.qazoService.getOrCreateQazoCount(telegramId);
    } catch (error) {
